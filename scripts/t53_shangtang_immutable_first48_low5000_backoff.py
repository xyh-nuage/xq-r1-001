...
def main() -> int:
    if len(sys.argv) != 4:
        raise SystemExit("usage: t53... PACKET PROMPT OUTPUT_DIR")
    packet = Path(sys.argv[1]).read_text(encoding="utf-8")
    system_prompt = Path(sys.argv[2]).read_text(encoding="utf-8").rstrip("\n")
    out = Path(sys.argv[3]); out.mkdir(parents=True, exist_ok=True)
    api_key = os.environ.get("LLM_API_KEY", "")
    model = os.environ.get("LLM_MODEL", "")
    source_sha = os.environ.get("SOURCE_SHA", "")
    packet_source_commit = os.environ.get("PACKET_SOURCE_COMMIT", "")
    if not all([api_key, model, source_sha, packet_source_commit]):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if int(exc.code) == 429 and attempt < max_attempts:
                ...
                continue
            raw = {
                "schema_version": "research_immutable_first48_low5000_backoff_raw_result.v1",
