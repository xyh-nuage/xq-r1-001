...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: t44_shangtang_compact_low_max1800_retry429.py SOURCE_ROOT OUTPUT_DIR")
    ...
    for attempt in (1, 2):
        ...
        try:
            ...
            attempts.append({"attempt": attempt, "http_status": int(exc.code), "result": "http_error", "body": body})
            if int(exc.code) == 429 and attempt == 1:
                print("FIRST_ATTEMPT_HTTP_429_WAITING_FOR_SINGLE_RETRY")
                time.sleep(RETRY_429_DELAY_SECONDS)
                continue
            failure = {
                "schema_version": "research_full_context_compact_low1800_raw_result.v1",
                ...
                "retry_policy": "one retry only after HTTP 429; 60 second delay",
                "attempts": attempts,
            ...
            attempts.append({"attempt": attempt, "result": "transport_or_response_error", "type": type(exc).__name__, "message": str(exc)})
            failure = {
                "schema_version": "research_full_context_compact_low1800_raw_result.v1",
                "status": "MODEL_CALL_FAILED_PRE_REFERENCE",
                "exact_source_commit": source_sha,
                ...
                "model_call_count": len(attempts),
                "retry_count": max(0, len(attempts) - 1),
                "retry_policy": "one retry only after HTTP 429; no retry for transport/other errors",
                "attempts": attempts,
                "silver_gold_reference_opened_before_this_file": False,
            }
            write_json(out / "full_context_raw_result.json", failure)
            print("TERMINAL_STATUS=TRANSPORT_OR_RESPONSE_ERROR")
            print("provider_attempts=" + str(len(attempts)))
            return 21
    ...
    print("max_tokens=1800")
    print("request_bytes=" + str(len(request_bytes)))
    print("packet_chars=" + str(stats.get("packet_chars", "")))
    print("collapsed_quote_bodies=" + str(stats.get("collapsed_quote_bodies", "")))
