...
def main() -> int:
    if len(sys.argv) != 3:
        raise SystemExit("usage: t37_shangtang_human_cluster_provider_default_one_call.py SOURCE_ROOT OUTPUT_DIR")
    ...
    import build_research_full_context_human_packet_vehicle_guard as human
    ...
    # Deliberately omit max_tokens. The provider/model decides its default output ceiling.
    request_body = {
        "model": model,
        "messages": [
    ...
    try:
        ...
        failure = {
            "schema_version": "research_full_context_human_cluster_raw_result.v2",
            ...
            "output_limit_mode": "provider_model_default",
    ...
    if isinstance(content, str):
        ...
        parse_error = "response content is not text"
    ...
    print("reasoning_tokens=" + str(((usage.get("completion_tokens_details") or {}).get("reasoning_tokens")) or ""))
