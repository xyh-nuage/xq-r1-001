...
def main() -> int:
    if len(sys.argv) != 4:
        raise SystemExit("usage: t54_shangtang_immutable_first48_nonthink5000_backoff.py PACKET PROMPT OUTPUT_DIR")
    ...
    for attempt in range(1, 5):
        ...
        try:
            ...
            if int(exc.code) == 429 and attempt <= len(BACKOFF_SECONDS):
                delay = BACKOFF_SECONDS[attempt - 1]
                print(f"HTTP_429_BACKOFF_{delay}S_BEFORE_ATTEMPT_{attempt + 1}")
                time.sleep(delay)
                continue
            write_json(out / "full_context_raw_result.json", {
                "schema_version": "research_immutable_first48_nonthink5000_backoff_raw_result.v1",
    ...
    if envelope is None:
        raise SystemExit("provider loop ended without success or terminal error")
